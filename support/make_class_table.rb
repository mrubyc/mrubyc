...
def write_file( all_classes )
  ...
  all_classes.each {|cls|
    ...
    file.puts "extern const struct #{cls[:struct_name]} mrbc_class_#{cls[:class]}_init;"
  }
...
EOL
  ...
  all_classes.each {|cls|
    cls_name_i = "MRBC_CLASS(#{cls[:class]}_init)"
    cls_name_t = "MRBC_CLASS(#{cls[:class]})"

    case cls[:class]
    when "Float", "String", "Math"
      file.puts "#if MRBC_USE_#{cls[:class].upcase}"
      file.puts "  { #{cls_name_i}, #{cls_name_t} },"
      file.puts "#endif"
    else
      file.puts "  { #{cls_name_i}, #{cls_name_t} },"
    end
